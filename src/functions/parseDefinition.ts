...
export function parseDefinition(
  propValue: CustomAttributeValue,
  propDefinition: ClassDefinition,
  prefix: string = ""
): string[] {
  if (
    propDefinition === null ||
    propDefinition === undefined ||
    propDefinition === false ||
    propDefinition === 0 ||
    propDefinition === ""
  ) {
    return [];
  ...
  if (typeof propDefinition === "string") {
  ...
  } else if (propDefinition instanceof Array) {
    //  prop: string[]
    if (propValue === true) {
    ...
    //  prop: {key: value?}  // propDefinition.toString() === "[object Object]"
